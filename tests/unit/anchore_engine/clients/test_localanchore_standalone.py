import pytest
from anchore_engine.clients.localanchore_standalone import retrying_pull_image, AnalysisError
...
def always_fail(*args, **kwargs):
    global fail_counter
    fail_counter += 1
    raise AnalysisError(msg='', pull_string='testing', tag='latest', cause=Exception('cannot pull'))
...
def fail_twice(*args, **kwargs):
    ...
    if fail_counter < fail_threshold:
        fail_counter += 1
        raise AnalysisError(msg='', pull_string='somepullstring', tag='latest', cause=Exception('cannot pull'))
    else:
        return True
...
def test_retrying_image_pull_full_failure(alwaysfail_pull):
    ...
    with pytest.raises(AnalysisError):
        retrying_pull_image(staging_dirs={}, pullstring='somepullstring', registry_creds=[], manifest=None, parent_manifest=None)
